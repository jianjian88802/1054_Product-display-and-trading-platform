                ...
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button
                                    v-if="isAuth('yonghu','加入黑名单') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1 && scope.row.heimd == 0"
                                    type="success" icon="el-icon-edit" size="mini" @click="jiaru(scope.row.id)">{{
                                contents.tableBtnFont == 1?'加入黑名单':'' }}
                            </el-button>
                            <el-button
                                    v-if="isAuth('yonghu','加入黑名单') && contents.tableBtnIcon == 1 && scope.row.heimd == 0 && contents.tableBtnIconPosition == 2"
                                    type="success" size="mini" @click="jiaru(scope.row.id)">{{ contents.tableBtnFont ==
                                1?'加入黑名单':'' }}<i class="el-icon-edit el-icon--right"/></el-button>
                            <el-button v-if="isAuth('yonghu','加入黑名单') && scope.row.heimd == 0 && contents.tableBtnIcon == 0" type="success"
                                       size="mini" @click="jiaru(scope.row.id)">{{ contents.tableBtnFont == 1?'加入黑名单':''
                                }}
                            </el-button>
    ...
    export default {
        ...
        methods: {
            ...
            ,
                ...
                jiaru(id)
                {
                    this.$confirm(`确定要将这个用户添加到黑名单吗?`, "提示", {
                        confirmButtonText: "确定",
                        cancelButtonText: "取消",
                        type: "warning"
                    }).then(() => {
                        this.$http({
                        url: "yonghu/jiaru?ids=" + id,
                        method: "get",
                    }).then(({data}) => {
                        if(data && data.code === 0
                )
